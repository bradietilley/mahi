import { EVENTS_TOKEN, ServiceProvider } from "@mahi/core";
 ...
 *
 * Entirely optional: `@mahi/http-client` works standalone with no
...
export class HttpClientServiceProvider extends ServiceProvider {
   ...
   *
   * Deferred to `boot()` because `@mahi/events` may register after this
   ...
   * so `@mahi/events` stays an optional peer.
   */
  boot(): void {
